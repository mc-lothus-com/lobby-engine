...
public class ServerMenu extends AbstractMenu {
    ...
    public void items(Player player) {
        ...
        inventory.setItem(12, new ItemCreator(Material.GRASS).setDisplayName("§aSky Wars").setLore(
                "§7" + (BukkitCore.getInstance().getOnlineCount(ServerType.LOBBY_SKYWARS) + BukkitCore.getInstance().getOnlineCount(ServerType.ROOM_SKYWARS)) + " jogando agora!",
                "§eClique para conectar-se."
        ).build());
        inventory.setItem(13, new ItemCreator(Material.BED).setDisplayName("§aBed Wars").setLore(
                "§7" + (BukkitCore.getInstance().getOnlineCount(ServerType.LOBBY_BEDWARS) + BukkitCore.getInstance().getOnlineCount(ServerType.ROOM_BEDWARS)) + " jogando agora!",
                "§eClique para conectar-se."
        ).build());
        inventory.setItem(14, new ItemCreator(Material.DIAMOND_CHESTPLATE).setDisplayName("§aTreinamento").setLore(
                "§7" + (BukkitCore.getInstance().getOnlineCount(ServerType.LOBBY_DUELS) + BukkitCore.getInstance().getOnlineCount(ServerType.ROOM_DUELS)) + " jogando agora!",
                "§eClique para conectar-se."
        ).build());
    }
